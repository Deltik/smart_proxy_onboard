...
class SmartProxyOnboardBmcScannerTest < Test::Unit::TestCase
  ...
  def test_fail_create_scanner_from_invalid_argument
    scanner = Proxy::Onboard::BMC::IPMIScanner.new('bogus')
    assert !scanner.valid?
  ...
  def test_should_scan_to_list_unthreaded
    expected = ['192.168.1.3', '192.168.1.5', '192.168.1.6']
    Proxy::Onboard::BMC::IPMIScanner.any_instance.stubs(:address_pings?).returns(false, true, false, true, true, false)
